...
def output(data, size):
    ...
    np.savetxt(f'data/{size}_alpha.csv', alpha, fmt=['%d', '%.5f'], delimiter=',', header='id,alpha')
    beta = np.c_[np.arange(data.numTasks), np.exp(data.beta)]
    np.savetxt(f'data/{size}_beta.csv', beta, fmt=['%d', '%.5f'], delimiter=',', header='id,beta')
    probZ = np.c_[np.arange(data.numTasks), data.probZ]
    np.savetxt(fname=f'data/{size}_probZ.csv',
               X=probZ,
    ...
    np.savetxt(f'data/{size}_label_glad.csv', label, fmt=['%d', '%d'], delimiter=',', header='id,label')
...
def main(filename=None, threshold=0.01):
    # global debug, verbose
    # debug = args.debug
    # if debug == True:
    #     verbose = True
